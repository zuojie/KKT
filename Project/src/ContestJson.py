...
getContestsJson：在线从json文件中获取比赛信息
...
class CContestMain(object):
	def __init__(self, hour_interval, contest_url = r"http://acm.nankai.edu.cn/recent_contests.php",\
		...
		self.contest_url_json = 'http://contests.acmicpc.info/contests.json'
		self.contests = []
		self.particitants = []
	...
	def getContestsJson(self):
		...
		#urllib2.install_opener(opener)
		#以上为开启控制台打印收发包内容		
		res = urllib2.urlopen(self.contest_url_json)
		page = res.read().decode('GBK').encode('utf-8') #网络数据以gbk解开，然后以utf-8编码成python处理用的格式
		#print page.decode('utf-8').encode('gbk')
	...
	def getParticitants(self):
		...
		cect = cdb.getConnect()
		cur = cect.cursor()
		sql = 'select * from user_info_participanterInfo'
		cur.execute(sql)
		res = cur.fetchall()
		for i in res:
			if i <> '':
	...
	def getFileContest(self):
		try:
			f = codecs.open(self.contest_file_name, 'r', 'gbk')
		except IOError:
			f = codecs.open(self.contest_file_name, 'w', 'gbk')
			f.close
			f = codecs.open(self.contest_file_name, 'r', 'gbk')
		data = f.read().split('\n')
		f.close()
	...
	def sendSMS(self, my_phone, contest, particitant):
		msg = contest.contestMsg().encode('utf-8')
...
if __name__ == "__main__":
	ccm = CContestMain(24 * 1) # 24小时之内的比赛进行提醒
	ccm.contestRun()
